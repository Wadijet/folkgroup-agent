...
func DoSyncIncrementalCustomers_v2() error {
	...
	log.Println("Bắt đầu đồng bộ customers đã cập nhật gần đây (incremental sync)...")
	err := integrations.BridgeV2_SyncNewCustomers()
	if err != nil {
		log.Printf("❌ Lỗi khi đồng bộ customers đã cập nhật gần đây: %v", err)
		return err
	}
	log.Println("Đồng bộ customers đã cập nhật gần đây thành công")
	return nil
}
