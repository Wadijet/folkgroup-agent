...
func (m *MetricsCollector) CollectJobStatuses() []JobStatus {
	...
	for jobName, job := range allJobs {
		...
		if ok {
			...
			// Xác định status dựa trên metrics
			if !isEnabled {
				status.Status = "disabled"
			} else if metrics.LastRunAt.IsZero() {
				status.Status = "idle"
			} else if metrics.LastRunStatus == "failed" {
				status.Status = "error"
			} else {
				status.Status = "idle" // Có thể mở rộng để check isRunning
			}
